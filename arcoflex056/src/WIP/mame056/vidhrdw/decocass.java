...
public class decocass {
	...
	static int[] color_center_bot=new int[1];
	static int[] mode_set=new int[1];
	...
	public static UBytePtr decocass_charram=new UBytePtr();
	public static UBytePtr decocass_fgvideoram=new UBytePtr();
	public static UBytePtr decocass_colorram=new UBytePtr();
	public static UBytePtr decocass_bgvideoram=new UBytePtr(); /* shares bits D0-3 with tileram! */
	public static UBytePtr decocass_tileram=new UBytePtr();
	public static UBytePtr decocass_objectram=new UBytePtr();
	
	public static int[] decocass_fgvideoram_size=new int[1];
	public static int[] decocass_colorram_size=new int[1];
	public static int[] decocass_bgvideoram_size=new int[1];
	public static int[] decocass_tileram_size=new int[1];
	public static int[] decocass_objectram_size=new int[1];
	
	static struct_tilemap fg_tilemap, bg_tilemap_l, bg_tilemap_r;
	
	static UBytePtr sprite_dirty;
	static UBytePtr char_dirty;
	static UBytePtr tile_dirty;
	static char object_dirty;
	
	static int tile_offset[] = {
	...
	public static GetMemoryOffsetPtr fgvideoram_scan_cols = new GetMemoryOffsetPtr() {
	...
	public static GetMemoryOffsetPtr bgvideoram_scan_cols = new GetMemoryOffsetPtr() {
            public int handler(int col, int row, int num_cols, int num_rows) {
		/* logical (col,row) . memory offset */
	...
	static GetTileInfoPtr get_bg_l_tile_info = new GetTileInfoPtr() {
            public void handler(int tile_index) {
                int color = (color_center_bot[0] >> 7) & 1;
		SET_TILE_INFO(
				2,
				(0x80 == (tile_index & 0x80)) ? 16 : decocass_bgvideoram.read(tile_index) >> 4,
				color,
				0);
            }
        };
	
	static GetTileInfoPtr get_bg_r_tile_info = new GetTileInfoPtr() {
            public void handler(int tile_index) {
		int color = (color_center_bot[0] >> 7) & 1;
		SET_TILE_INFO(
				2,
				(0x00 == (tile_index & 0x80)) ? 16 : decocass_bgvideoram.read(tile_index) >> 4,
				color,
				TILE_FLIPY);
            }
        };
	...
	static GetTileInfoPtr get_fg_tile_info = new GetTileInfoPtr() {
            public void handler(int tile_index) {
		int code = decocass_fgvideoram.read(tile_index);
		int attr = decocass_colorram.read(tile_index);
                
		SET_TILE_INFO(
				0,
				256 * (attr & 3) + code,
				color_center_bot[0] & 1,
				0);
            }
        };
	...
	static void draw_object(mame_bitmap bitmap)
	{
		int sx, sy, color;
	
		if (0 == (mode_set[0] & 0x80))  /* part_h_enable off? */
			return;
	
		color = (color_center_bot[0] >> 4) & 15;
	...
	static void draw_center(mame_bitmap bitmap)
	{
		int sx, sy, x, y, color;
	
		color = 0;
		if ((color_center_bot[0] & 0x10) != 0)
			color |= 4;
		if ((color_center_bot[0] & 0x20) != 0)
			color |= 2;
		if ((color_center_bot[0] & 0x40) != 0)
			color |= 1;
		if ((color_center_bot[0] & 0x80) != 0)
		...
		for (y = 0; y < 4; y++)
		{
			if (((sy + y) & color_center_bot[0] & 3) == (sy & color_center_bot[0] & 3))
				for (x = 0; x < 256; x++)
					if (0 != (x & 16) || 0 != (center_h_shift_space & 1))
						plot_pixel.handler(bitmap, (sx + x) & 255, sy + y, Machine.pens[color]);
		}
	}
	...
	{
		if (data == decocass_charram.read(offset))
			return;
		decocass_charram.write(offset, data);
		/* dirty sprite */
		sprite_dirty.write((offset >> 5) & 255, 1);
		/* dirty char */
		char_dirty.write((offset >> 3) & 1023, 1);
	} };
	
	
	public static WriteHandlerPtr decocass_fgvideoram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		if (data == decocass_fgvideoram.read(offset))
			return;
		decocass_fgvideoram.write(offset, data);
		tilemap_mark_tile_dirty( fg_tilemap, offset );
	} };
	
	public static WriteHandlerPtr decocass_colorram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		if (data == decocass_colorram.read(offset))
			return;
		decocass_colorram.write(offset, data);
		tilemap_mark_tile_dirty( fg_tilemap, offset );
	} };
	
	static void mark_bg_tile_dirty(int offset)
	{
		if ((offset & 0x80) != 0)
	...
	{
		if (data == decocass_tileram.read(offset))
			return;
		decocass_tileram.write(offset, data);
		/* dirty tile (64 bytes per tile) */
		tile_dirty.write((offset / 64) & 15, 1);
		/* first 1KB of tile RAM is shared with tilemap RAM */
		if (offset < decocass_bgvideoram_size[0])
	...
	{
		if (data == decocass_objectram.read(offset))
			return;
		decocass_objectram.write(offset, data);
		/* dirty the object */
		object_dirty = 1;
	} };
	...
	{
		if (data == decocass_bgvideoram.read(offset))
			return;
		decocass_bgvideoram.write(offset, data);
		mark_bg_tile_dirty( offset );
	} };
	...
	public static ReadHandlerPtr decocass_mirrorvideoram_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		return decocass_fgvideoram.read(offset);
	} };
	...
	public static ReadHandlerPtr decocass_mirrorcolorram_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		return decocass_colorram.read(offset);
	} };
	...
	public static WriteHandlerPtr decocass_mirrorvideoram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		decocass_fgvideoram_w.handler(offset,data);
	} };
	...
	public static WriteHandlerPtr decocass_mirrorcolorram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		decocass_colorram_w.handler(offset,data);
	} };
	...
	static void draw_sprites(mame_bitmap bitmap, int color,
	...
	static void draw_missiles(mame_bitmap bitmap,
							int missile_y_adjust, int missile_y_adjust_flip_screen,
							UBytePtr missile_ram, int interleave)
	{
		int i,offs;
		...
		for (i = 0, offs = 0; i < 8; i++, offs += 4*interleave)
		{
			...
			sy = 255 - missile_ram.read(offs + 0*interleave);
			sx = 255 - missile_ram.read(offs + 2*interleave);
			if (flip_screen()!=0)
			{
				sx = 240 - sx;
			...
			sy = 255 - missile_ram.read(offs + 1*interleave);
			sx = 255 - missile_ram.read(offs + 3*interleave);
			if (flip_screen() != 0)
			{
				sx = 240 - sx;
				sy = 240 - sy + missile_y_adjust_flip_screen;
	...
	static void decode_modified(UBytePtr sprite_ram, int interleave)
	{
		...
		for (offs = decocass_fgvideoram_size[0] - 1;offs >= 0;offs--)
		{
			int code;
	
			code = decocass_fgvideoram.read(offs) + 256 * (decocass_colorram.read(offs) & 3);
	
			switch (char_dirty.read(code))
			{
			case 1:
		...
		for (i = 0; i < 1024; i++)
		{
			if (char_dirty.read(i) == 2)
				char_dirty.write(i, 0);
		}
		...
		/* decode dirty tiles */
		for (offs = 0; offs < decocass_bgvideoram_size[0]; offs++)
		{
			int code = (decocass_tileram.read(offs) >> 4) & 15;
			...
			if (tile_dirty.read(code) != 0)
			{
				...
				for (i = offs; i < decocass_bgvideoram_size[0]; i++)
					if (code == ((decocass_tileram.read(i) >> 4) & 15))
	...
	public static VhStartPtr decocass_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		if (null == (sprite_dirty = new UBytePtr(256)) ||
			null == (char_dirty = new UBytePtr(1024)) ||
			null == (tile_dirty = new UBytePtr(16)))
			return 1;
		...
		if (bg_tilemap_l==null || bg_tilemap_r==null || fg_tilemap==null)
			return 1;
		...
		decocass_bgvideoram_size[0] = 0x0400;	/* d000-d3ff */
	
		/*TODO*///state_save_register_int("decocass", 0, "watchdog_count", &watchdog_count);
		/*TODO*///state_save_register_int("decocass", 0, "watchdog_flip", &watchdog_flip);
		/*TODO*///state_save_register_int("decocass", 0, "color_missiles", &color_missiles);
		/*TODO*///state_save_register_int("decocass", 0, "color_center_bot", &color_center_bot);
		/*TODO*///state_save_register_int("decocass", 0, "mode_set", &mode_set);
		/*TODO*///state_save_register_int("decocass", 0, "back_h_shift", &back_h_shift);
		/*TODO*///state_save_register_int("decocass", 0, "back_vl_shift", &back_vl_shift);
		/*TODO*///state_save_register_int("decocass", 0, "back_vr_shift", &back_vr_shift);
		/*TODO*///state_save_register_int("decocass", 0, "part_h_shift", &part_h_shift);
		/*TODO*///state_save_register_int("decocass", 0, "part_v_shift", &part_v_shift);
		/*TODO*///state_save_register_int("decocass", 0, "center_h_shift_space", &center_h_shift_space);
		/*TODO*///state_save_register_int("decocass", 0, "center_v_shift", &center_v_shift);
	...
	public static VhUpdatePtr decocass_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
	{
		...
		if (0xc0 != (input_port_2_r.handler(0) & 0xc0))  /* coin slots assert an NMI */
	...
	/*TODO*///	{
	/*TODO*///		static int showmsg;
	/*TODO*///		if (code_pressed_memory(KEYCODE_I))
	/*TODO*///			showmsg ^= 1;
	/*TODO*///		if (showmsg)
	/*TODO*///			usrintf_showmessage_secs(1, "mode:$%02x cm:$%02x ccb:$%02x h:$%02x vl:$%02x vr:$%02x ph:$%02x pv:$%02x ch:$%02x cv:$%02x",
	/*TODO*///				mode_set,
	/*TODO*///				color_missiles,
	/*TODO*///				color_center_bot,
	/*TODO*///				back_h_shift,
	/*TODO*///				back_vl_shift,
	/*TODO*///				back_vr_shift,
	/*TODO*///				part_h_shift,
	/*TODO*///				part_v_shift,
	/*TODO*///				center_h_shift_space,
	/*TODO*///				center_v_shift);
	/*TODO*///	}
	/*TODO*///#endif
		...
		if (0 == (mode_set[0] & 0x02))
			scrollx += 256;
	
	/*TODO*///#if 0
	/*TODO*////* this is wrong */
	/*TODO*///	if (0 != back_h_shift && 0 == ((mode_set ^ (mode_set >> 1)) & 1))
	/*TODO*///		scrollx += 256;
	/*TODO*///#endif
	
		if (0 == (mode_set[0] & 0x04))
			scrolly_r += 256;
		else
			scrolly_l += 256;
		...
		if ((mode_set[0] & 0x20) != 0)
		{
			if ((mode_set[0] & 0x08) != 0)	/* bkg_ena on ? */
			{
		...
		draw_sprites(bitmap, (color_center_bot[0] >> 1) & 1, 0, 0, decocass_fgvideoram, 0x20);
