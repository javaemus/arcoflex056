...
public class srumbler
{
	...
	public static UBytePtr srumbler_backgroundram=new UBytePtr(), srumbler_foregroundram=new UBytePtr();
	...
	***************************************************************************/
	...
	static GetTileInfoPtr get_fg_tile_info = new GetTileInfoPtr() {
            public void handler(int tile_index) {
                int attr = srumbler_foregroundram.read(2*tile_index);
		SET_TILE_INFO(
				0,
				srumbler_foregroundram.read(2*tile_index + 1) + ((attr & 0x03) << 8),
				(attr & 0x3c) >> 2,
	...
	static GetTileInfoPtr get_bg_tile_info = new GetTileInfoPtr() {
            public void handler(int tile_index) {
		int attr = srumbler_backgroundram.read(2*tile_index);
		SET_TILE_INFO(
				1,
				srumbler_backgroundram.read(2*tile_index + 1) + ((attr & 0x07) << 8),
				(attr & 0xe0) >> 5,
				TILE_SPLIT((attr & 0x10) >> 4) | ((attr & 0x08)!=0 ? TILE_FLIPY : 0));
            }
        };
	...
	***************************************************************************/
	...
	public static VhEofCallbackPtr srumbler_eof_callback = new VhEofCallbackPtr() {
            public void handler() {
