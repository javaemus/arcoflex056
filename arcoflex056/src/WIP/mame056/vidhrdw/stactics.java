...
import static WIP.mame056.machine.stactics.stactics_horiz_pos;
import static WIP.mame056.machine.stactics.stactics_motor_on;
import static WIP.mame056.machine.stactics.stactics_vert_pos;
...
public class stactics
{
	...
	public static UBytePtr dirty_videoram_b = new UBytePtr();
	public static UBytePtr dirty_chardata_b = new UBytePtr();
	public static UBytePtr dirty_videoram_d = new UBytePtr();
	public static UBytePtr dirty_chardata_d = new UBytePtr();
	public static UBytePtr dirty_videoram_e = new UBytePtr();
	public static UBytePtr dirty_chardata_e = new UBytePtr();
	public static UBytePtr dirty_videoram_f = new UBytePtr();
	public static UBytePtr dirty_chardata_f = new UBytePtr();
	...
	public static UBytePtr beamdata = new UBytePtr();
	...
	public static VhConvertColorPromPtr stactics_vh_convert_color_prom = new VhConvertColorPromPtr() {
            public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
                ...
                for (i=0;i<16;i++)
                {
                    ...
                    /* red component */
                    palette[_palette++] = (char) (0xff * bit0);

                    /* green component */
                    palette[_palette++] = (char) (0xff * bit1 - 0xcc * bit3);

                    /* blue component */
                    palette[_palette++] = (char) (0xff * bit2);
                }
                ...
                for(i=0;i<4;i++)
                {
                    ...
                    for(j=0;j<16;j++)
                    {
                        ...
                        colortable[_colortable++] = color_prom.read(i*0x100+0x01*0x10+j);
                    }
                    /* For page F - Close Aliens (these are all the same color) */
                    for(j=0;j<16;j++)
                    {
                        colortable[_colortable++] = 0;
                        colortable[_colortable++] = color_prom.read(i*0x100+0x02*0x10);
                    }
                    /* For page E - Medium Aliens (these are all the same color) */
                    for(j=0;j<16;j++)
                    {
                        colortable[_colortable++] = 0;
                        colortable[_colortable++] = color_prom.read(i*0x100+0x04*0x10+j);
                    }
                    /* For page D - Far Aliens (these are all the same color) */
                    for(j=0;j<16;j++)
                    {
                        colortable[_colortable++] = 0;
                        colortable[_colortable++] = color_prom.read(i*0x100+0x08*0x10+j);
                    }
                }
            }
        };
	...
	***************************************************************************/
	...
	{
	    int i,j;
	    UBytePtr firebeam_data = new UBytePtr();
	    UBytePtr firechar=new UBytePtr(256*8*9);
	
	    if ((tmpbitmap  = bitmap_alloc(Machine.drv.screen_width,Machine.drv.screen_height)) == null) return 1;
	    if ((tmpbitmap2 = bitmap_alloc(Machine.drv.screen_width,Machine.drv.screen_height)) == null) return 1;
	    if ((bitmap_B = bitmap_alloc(Machine.drv.screen_width,Machine.drv.screen_height)) == null)   return 1;
	    if ((bitmap_D = bitmap_alloc(Machine.drv.screen_width,Machine.drv.screen_height)) == null)   return 1;
	    if ((bitmap_E = bitmap_alloc(Machine.drv.screen_width,Machine.drv.screen_height)) == null)   return 1;
	    if ((bitmap_F = bitmap_alloc(Machine.drv.screen_width,Machine.drv.screen_height)) == null)   return 1;
	
		/* Allocate dirty buffers */
		if ((dirty_videoram_b = new UBytePtr(videoram_size[0])) == null)       return 1;
		if ((dirty_videoram_d = new UBytePtr(videoram_size[0])) == null)       return 1;
		if ((dirty_videoram_e = new UBytePtr(videoram_size[0])) == null)       return 1;
		if ((dirty_videoram_f = new UBytePtr(videoram_size[0])) == null)       return 1;
		if ((dirty_chardata_b = new UBytePtr(DIRTY_CHARDATA_SIZE)) == null) return 1;
		if ((dirty_chardata_d = new UBytePtr(DIRTY_CHARDATA_SIZE)) == null) return 1;
		if ((dirty_chardata_e = new UBytePtr(DIRTY_CHARDATA_SIZE)) == null) return 1;
		if ((dirty_chardata_f = new UBytePtr(DIRTY_CHARDATA_SIZE)) == null) return 1;
	    ...
	    for(i=0;i<256;i++)
	    {
	        for(j=0;j<8;j++)
	        {
	            if (((i>>j)&0x01) != 0)
	            {
	                firechar.write(i*9+(7-j), firechar.read(i*9+(7-j))   | (0x01<<(7-j)) );
	                firechar.write(i*9+(7-j)+1, firechar.read(i*9+(7-j)+1) | (0x01<<(7-j)) );
	            }
	        }
	    }
		...
		if ((beamdata = new UBytePtr(BEAMDATA_SIZE)) == null) return 1;
	    ...
	    stactics_motor_on = new UBytePtr(1024);
	...
	***************************************************************************/
	...
	public static WriteHandlerPtr stactics_videoram_b_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_videoram_b.read(offset) != data)
	    {
	        ...
	        dirty_videoram_b.write(offset, 1);
	    }
	} };
	...
	public static WriteHandlerPtr stactics_chardata_b_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_chardata_b.read(offset) != data)
	    {
	        ...
	        dirty_chardata_b.write(offset>>3, 1);
	    }
	} };
	...
	public static WriteHandlerPtr stactics_videoram_d_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_videoram_d.read(offset) != data)
	    {
	        ...
	        dirty_videoram_d.write(offset, 1);
	    }
	} };
	...
	public static WriteHandlerPtr stactics_chardata_d_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_chardata_d.read(offset) != data)
	    {
	        ...
	        dirty_chardata_d.write(offset>>3, 1);
	    }
	} };
	...
	public static WriteHandlerPtr stactics_videoram_e_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_videoram_e.read(offset) != data)
	    {
	        ...
	        dirty_videoram_e.write(offset, 1);
	    }
	} };
	...
	public static WriteHandlerPtr stactics_chardata_e_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_chardata_e.read(offset) != data)
	    {
	        ...
	        dirty_chardata_e.write(offset>>3, 1);
	    }
	} };
	...
	public static WriteHandlerPtr stactics_videoram_f_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_videoram_f.read(offset) != data)
	    {
	        ...
	        dirty_videoram_f.write(offset, 1);
	    }
	} };
	...
	public static WriteHandlerPtr stactics_chardata_f_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	    if (stactics_chardata_f.read(offset) != data)
	    {
	        ...
	        dirty_chardata_f.write(offset>>3, 1);
	    }
	} };
	...
	***************************************************************************/
	...
	public static VhUpdatePtr stactics_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
	{
	    ...
	    for(offs=0x400-1; offs>=0; offs--)
	    {
	        ...
	        if (dirty_chardata_d.read(char_number) == 1)
	        {
	            decodechar(Machine.gfx[3],
	                       char_number,
	                       stactics_chardata_d,
	                       Machine.drv.gfxdecodeinfo[3].gfxlayout);
	            dirty_chardata_d.write(char_number, 2);
	            dirty_videoram_d.write(offs, 1);
	        }
	        else if (dirty_chardata_d.read(char_number) == 2)
	        {
	            dirty_videoram_d.write(offs, 1);
	        }
	
	        if (dirty_videoram_d.read(offs) != 0)
	        {
	        ...
	        if (dirty_chardata_e.read(char_number) == 1)
	        {
	            decodechar(Machine.gfx[2],
	                       char_number,
	                       stactics_chardata_e,
	                       Machine.drv.gfxdecodeinfo[2].gfxlayout);
	            dirty_chardata_e.write(char_number, 2);
	            dirty_videoram_e.write(offs, 1);
	        }
	        else if (dirty_chardata_e.read(char_number) == 2)
	        {
	            dirty_videoram_e.write(offs, 1);
	        }
	
	        if (dirty_videoram_e.read(offs) != 0)
	        {
	        ...
	        if (dirty_chardata_f.read(char_number) == 1)
	        {
	            decodechar(Machine.gfx[1],
	                       char_number,
	                       stactics_chardata_f,
	                       Machine.drv.gfxdecodeinfo[1].gfxlayout);
	            dirty_chardata_f.write(char_number, 2);
	            dirty_videoram_f.write(offs, 1);
	        }
	        else if (dirty_chardata_f.read(char_number) == 2)
	        {
	            dirty_videoram_f.write(offs, 1);
	        }
	
	        if (dirty_videoram_f.read(offs) != 0)
	        {
	        ...
	        if (dirty_chardata_b.read(char_number) == 1)
	        {
	            decodechar(Machine.gfx[0],
	                       char_number,
	                       stactics_chardata_b,
	                       Machine.drv.gfxdecodeinfo[0].gfxlayout);
	            dirty_chardata_b.write(char_number, 2);
	            dirty_videoram_b.write(offs, 1);
	        }
	        else if (dirty_chardata_b.read(char_number) == 2)
	        {
	            dirty_videoram_b.write(offs, 1);
	        }
	
	        if (dirty_videoram_b.read(offs) != 0)
	        {
	    ...
	    copyscrollbitmap(tmpbitmap2,bitmap_D,0,new int[]{0},1,new int[]{d_offset},
	                     Machine.visible_area,TRANSPARENCY_NONE,0);
	    copyscrollbitmap(tmpbitmap2,bitmap_E,0,new int[]{0},1,new int[]{e_offset},
	                     Machine.visible_area,TRANSPARENCY_COLOR,0);
	    copyscrollbitmap(tmpbitmap2,bitmap_F,0,new int[]{0},1,new int[]{f_offset},
	                     Machine.visible_area,TRANSPARENCY_COLOR,0);
	    ...
	    for(i=0;i<0xff;i++)
	    {
	        dirty_chardata_b.write(i, dirty_chardata_b.read(i) & 0x01);
	        dirty_chardata_d.write(i, dirty_chardata_d.read(i) & 0x01);
	        dirty_chardata_e.write(i, dirty_chardata_e.read(i) & 0x01);
	        dirty_chardata_f.write(i, dirty_chardata_f.read(i) & 0x01);
	    }
