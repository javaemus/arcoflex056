...
public class combatsc
{
	...
	public static UBytePtr combasc_scrollram = new UBytePtr();
	...
	***************************************************************************/
	...
	static GetTileInfoPtr get_tile_info0_bootleg = new GetTileInfoPtr() {
            public void handler(int tile_index) {
		int attributes = combasc_page[0].read(tile_index);
		int bank = 4*((combasc_vreg & 0x0f) - 1);
	...
	static GetTileInfoPtr get_tile_info1_bootleg = new GetTileInfoPtr() {
            public void handler(int tile_index) {
		int attributes = combasc_page[1].read(tile_index);
		int bank = 4*((combasc_vreg >> 4) - 1);
		int number, pal, color;
	...
	***************************************************************************/
	...
	static void draw_sprites(mame_bitmap bitmap, UBytePtr source,int circuit,int pri_mask)
	{
	...
	public static VhUpdatePtr combasc_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int fullrefresh) 
	{
		...
		if (priority == 0)
		{
			...
			draw_sprites(bitmap,new UBytePtr(private_spriteram[1]),1,0x0f00);
			draw_sprites(bitmap,new UBytePtr(private_spriteram[0]),0,0x4444);
		}
		else
		{
	...
	***************************************************************************/
	...
	static void bootleg_draw_sprites( mame_bitmap bitmap, UBytePtr source, int circuit )
	{
		...
		if( limit>=0 ) finish = new UBytePtr(source, -limit*8);
		source.dec(8);
