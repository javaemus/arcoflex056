...
import static WIP.mame056.vidhrdw.argus.*;
...
public class argus
{
	...
	***************************************************************************/
	...
	static int butasan_page_latch = 0x00;
	...
	***************************************************************************/
	...
	static YM2203interface argus_ym2203_interface = new YM2203interface
	(
		1,				/* 1 chip  */
		6000000 / 4, 	/* 1.5 MHz */
		new int[]{ YM2203_VOL(50,15) },
	...
	static YM2203interface valtric_ym2203_interface = new YM2203interface
	(
		2,				/* 2 chips */
		6000000 / 4, 	/* 1.5 MHz */
	...
	static YM2203interface butasan_ym2203_interface = new YM2203interface
	(
		2,				/* 2 chips */
		6000000 / 4, 	/* 1.5 MHz */
		new int[]{ YM2203_VOL(100,30), YM2203_VOL(100,30) },
		new ReadHandlerPtr[]{ null, null },
		new ReadHandlerPtr[]{ null, null },
		new WriteHandlerPtr[]{ null, null },
		new WriteHandlerPtr[]{ null, null },
	...
	***************************************************************************/
	...
	{
		if ((butasan_page_latch & 0x01)==0)
		{
			if (offset < 0x0800)		/* BG0 RAM */
			{
				return butasan_bg0ram_r.handler( offset );
			}
			else if (offset < 0x1000)	/* Back BG0 RAM */
			{
				return butasan_bg0backram_r.handler( offset - 0x0800 );
			}
		}
		else
		{
			...
			{
				return butasan_txram_r.handler( offset );
			}
			else if (offset < 0x1000)	/* Back text RAM */
			{
				return butasan_txbackram_r.handler( offset - 0x0800 );
			}
		}
	...
	public static WriteHandlerPtr butasan_pagedram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		if ((butasan_page_latch & 0x01)==0)
		{
			if (offset < 0x0800)		/* BG0 RAM */
			{
				butasan_bg0ram_w.handler( offset, data );
			}
			else if (offset < 0x1000)	/* Back BG0 RAM */
			{
				butasan_bg0backram_w.handler( offset - 0x0800, data );
			}
		}
		...
		else
		{
			if (offset < 0x0800)		/* Text RAM */
			{
				butasan_txram_w.handler( offset, data );
			}
			else if (offset < 0x1000)	/* Back text RAM */
			{
				butasan_txbackram_w.handler( offset - 0x0800, data );
			}
		}
	} };
