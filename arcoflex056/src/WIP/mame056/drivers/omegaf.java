...
import static WIP.mame056.vidhrdw.omegaf.*;
...
public class omegaf
{
	...
	**************************************************************************/
	...
	public static InitDriverPtr init_omegaf = new InitDriverPtr() { public void handler()
	{
		...
		RAM.write(0x029a, 0x00);
		RAM.write(0x029b, 0x00);
		RAM.write(0x02a6, 0x00);
		RAM.write(0x02a7, 0x00);
	
		RAM.write(0x02b2, 0xC9);
		RAM.write(0x02b5, 0xC9);
		RAM.write(0x02c9, 0xC9);
		RAM.write(0x02f6, 0xC9);
	
		RAM.write(0x05f0, 0x00);
		RAM.write(0x054c, 0x04);
		RAM.write(0x0557, 0x03);
	
	
		/* Fix ROM check */
	
		RAM.write(0x0b8d, 0x00);
		RAM.write(0x0b8e, 0x00);
		RAM.write(0x0b8f, 0x00);
	} };
	...
	**************************************************************************/
	...
	public static WriteHandlerPtr omegaf_bankselect_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ( (data & 0x0f) != omegaf_bank_latch )
		{
			...
			cpu_setbank( 1, new UBytePtr(RAM, bankaddress) );	 /* Select 16 banks of 16k */
		}
	} };
	...
	**************************************************************************/
	...
	(
		2,	 /* 2 chips */
		12000000/8,
		new int[]{ YM2203_VOL(35, 35), YM2203_VOL(35, 35)},
	...
	(
		...
		32*8, 28*8, new rectangle( 0*8, 32*8-1, 4*8, 28*8-1 ),
