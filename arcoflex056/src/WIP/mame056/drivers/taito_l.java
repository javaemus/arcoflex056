...
import static WIP.mame056.vidhrdw.taito_l.*;
...
import static mame056.cpu.z80.z80H.Z80_IM;
...
public class taito_l
{
	...
	static taitol_HandlerPtr rambank_modify_notifiers[]=
	{
	...
	static taitol_HandlerPtr[] current_notifier = new taitol_HandlerPtr[4];
	public static UBytePtr[] current_base = new UBytePtr[4];
	...
	public static UBytePtr taitol_rambanks = new UBytePtr();
	
	static UBytePtr palette_ram = new UBytePtr();
	static UBytePtr empty_ram = new UBytePtr();
	...
	static taitol_HandlerPtr palette_notifier = new taitol_HandlerPtr() {
		...
		int b = (byte1 & 0xf) * 0x11;
		int g = ((byte0 & 0xf0)>>4) * 0x11;
		int r = (byte0 & 0xf) * 0x11;
	
		//	addr &= 0x1ff;
	
		if(addr > 0x200)
		{
                    logerror("Large palette ? %03x (%04x)n", addr, cpu_get_pc());
		}
		else
		{
	...
	public static InitMachinePtr machine_init = new InitMachinePtr() { public void handler()
	{
		...
		{
			cur_rambank[i] = 0x80;
			current_base[i] = new UBytePtr(palette_ram);
			current_notifier[i] = palette_notifier;
			cpu_setbank(2+i, new UBytePtr(current_base[i]));
		}
	...
	static InitMachinePtr fhawk_init = new InitMachinePtr() { public void handler()
	{
	...
	static InitMachinePtr raimais_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
		porte0_r = null;
		porte1_r = null;
	...
	static InitMachinePtr champwr_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
		porte0_r = null;
		porte1_r = null;
	...
	static InitMachinePtr kurikint_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
		porte0_r = null;
		porte1_r = null;
	...
	static InitMachinePtr puzznic_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
	...
	static InitMachinePtr plotting_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
		porte0_r = input_port_0_r;
		porte1_r = input_port_1_r;
	...
	static InitMachinePtr palamed_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
		porte0_r = input_port_0_r;
		porte1_r = null;
	...
	static InitMachinePtr cachat_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
		porte0_r = input_port_0_r;
		porte1_r = null;
	...
	static InitMachinePtr horshoes_init = new InitMachinePtr() { public void handler()
	{
		machine_init.handler();
		porte0_r = input_port_0_r;
		porte1_r = input_port_1_r;
	...
	public static InterruptPtr vbl_interrupt = new InterruptPtr() { public int handler() 
	{
		...
		if (cpunum_get_reg(0,Z80_IM) != 2) return ignore_interrupt.handler();
	...
	{
	//logerror("irq_adr_table[%d] = %02xn",offset,data);
	...
	public static WriteHandlerPtr rombankswitch_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if(cur_rombank != data)
		{
			...
			{
				high = data;
				logerror("New rom size : %xn", (high+1)*0x2000);
			}
	
	//		logerror("robs %d, %02x (%04x)n", offset, data, cpu_get_pc());
			cur_rombank = data;
	...
	public static WriteHandlerPtr rombank2switch_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if(cur_rombank2 != data)
		{
			...
			{
				high = data;
				logerror("New rom2 size : %xn", (high+1)*0x4000);
			}
	
	//		logerror("robs2 %02x (%04x)n", data, cpu_get_pc());
	...
	//logerror("rabs %d, %02x (%04x)n", offset, data, cpu_get_pc());
			...
			{
                                logerror("unknown rambankswitch %d, %02x (%04x)n", offset, data, cpu_get_pc());
	...
	{
		if (extport == 0) return porte0_r.handler(0);
		else return porte1_r.handler(0);
	} };
	
	public static ReadHandlerPtr portB_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		if (extport == 0) return portf0_r.handler(0);
		else return portf1_r.handler(0);
	} };
	...
	//	logerror("mcu write %02x (%04x)n", data, cpu_get_pc());
		switch(data)
		{
		case 0x43:
			mcu_pos = 0;
			mcu_reply = puzznic_mcu_reply;
			mcu_reply_len = puzznic_mcu_reply.length;
			break;
		}
	} };
	...
	{
	//	logerror("mcu control %02x (%04x)n", data, cpu_get_pc());
	} };
	
	public static ReadHandlerPtr mcu_data_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
	//	logerror("mcu read (%04x) [%02x, %04x]n", cpu_get_pc(), last_data, last_data_adr);
	...
	public static ReadHandlerPtr mux_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		default:
			logerror("Mux read from unknown port %d (%04x)n", mux_ctrl, cpu_get_pc());
			return 0xff;
		}
	} };
	...
	public static WriteHandlerPtr mux_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		case 4:
			...
			break;
		default:
			logerror("Mux write to unknown port %d, %02x (%04x)n", mux_ctrl, data, cpu_get_pc());
		}
	} };
	...
	public static WriteHandlerPtr champwr_adpcm_hi_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		while (i < romlen && (rom.read(i)!=0 || rom.read(i+1)!=0 || rom.read(i+2)!=0 || rom.read(i+3)!=0))
			i += 4;
	...
	public static Memory_ReadAddress fhawk_readmem[]={
		...
		new Memory_ReadAddress( 0xd000, 0xdfff, MRA_BANK3 ),			
		new Memory_ReadAddress( 0xe000, 0xefff, MRA_BANK4 ),			
		new Memory_ReadAddress( 0xf000, 0xfdff, MRA_BANK5 ),			
		new Memory_ReadAddress( 0xfe00, 0xfe03, taitol_bankc_r ),		
		new Memory_ReadAddress( 0xfe04, 0xfe04, taitol_control_r ),	
		new Memory_ReadAddress( 0xff00, 0xff02, irq_adr_r ),			
		new Memory_ReadAddress( 0xff03, 0xff03, irq_enable_r ),		
		new Memory_ReadAddress( 0xff04, 0xff07, rambankswitch_r ),	
		new Memory_ReadAddress( 0xff08, 0xff08, rombankswitch_r ),
	...
	public static Memory_WriteAddress fhawk_writemem[]={
		...
		new Memory_WriteAddress( 0xc000, 0xcfff, bank0_w ),			
		new Memory_WriteAddress( 0xd000, 0xdfff, bank1_w ),			
		new Memory_WriteAddress( 0xe000, 0xefff, bank2_w ),			
		new Memory_WriteAddress( 0xf000, 0xfdff, bank3_w ),			
		new Memory_WriteAddress( 0xfe00, 0xfe03, taitol_bankc_w ),		
		new Memory_WriteAddress( 0xfe04, 0xfe04, taitol_control_w ),	
		new Memory_WriteAddress( 0xff00, 0xff02, irq_adr_w ),			
		new Memory_WriteAddress( 0xff03, 0xff03, irq_enable_w ),		
		new Memory_WriteAddress( 0xff04, 0xff07, rambankswitch_w ),	
		new Memory_WriteAddress( 0xff08, 0xff08, rombankswitch_w ),
...
/*TODO*///	#if 0
/*TODO*///		new Memory_ReadAddress( 0xd000, 0xd000, input_port_0_r ),
/*TODO*///		new Memory_ReadAddress( 0xd001, 0xd001, input_port_1_r ),
/*TODO*///		new Memory_ReadAddress( 0xd002, 0xd002, input_port_2_r ),
/*TODO*///		new Memory_ReadAddress( 0xd003, 0xd003, input_port_3_r ),
/*TODO*///		new Memory_ReadAddress( 0xd007, 0xd007, input_port_4_r ),
/*TODO*///	#endif
		new Memory_ReadAddress(MEMPORT_MARKER, 0)
	};
...
/*TODO*///	#if 0
/*TODO*///		new Memory_WriteAddress( 0xc000, 0xc000, rombank2switch_w ),
/*TODO*///	#endif
		new Memory_WriteAddress(MEMPORT_MARKER, 0)
	};
	...
	public static Memory_ReadAddress puzznic_readmem[]={
		...
		new Memory_ReadAddress( 0xa001, 0xa001, ym2203_data0_r ),			
		new Memory_ReadAddress( 0xa003, 0xa003, ym2203_data1_r ),			
		new Memory_ReadAddress( 0x8000, 0x9fff, MRA_RAM ),
	...
	***********************************************************/
	...
	public static void TAITO_COINAGE_WORLD_8() {
	...
	public static void TAITO_COINAGE_JAPAN_NEW_8() {
		PORT_DIPNAME( 0x30, 0x30, DEF_STR( "Coin_A") ); 
	...
	public static void TAITO_COINAGE_US_8() {
	...
	public static void TAITO_L_PLAYERS_INPUT( int player ) {
		PORT_START();  
	...
	public static void  TAITO_L_SYSTEM_INPUT( int type, int impulse ) {
		PORT_START();  
	...
	public static void TAITO_L_DSWA_2_4() {
	...
	public static void CHAMPWR_DSWB() {
		PORT_START();  
		TAITO_DIFFICULTY_8();
	...
	public static void CHAMPWR_INPUTS() {
	 	PORT_START();  
	...
	public static WriteHandlerPtr portA_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if (cur_bank != (data & 0x03) )
		{
			...
			//logerror ("YM2203 bank change val=%02x  pc=%04xn",cur_bank, cpu_get_pc() );
		}
	} };
