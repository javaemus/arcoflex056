...
import static WIP.mame056.vidhrdw.suna8.*;
...
public class suna8
{
	...
	***************************************************************************/
	...
	public static InitDriverPtr init_hardhead = new InitDriverPtr() { public void handler() 
	{
		...
		{
			int x = RAM.read(i);
			if (   ( (i & 0x800)!=0 && ((~i & 0x400) ^ ((i & 0x4000)>>4))!=0 ) ? false:true	)
			{
				x	=	x ^ 0x58;
				x	=	(((x & (1<<0))!=0?1:0)<<0) |
						(((x & (1<<1))!=0?1:0)<<1) |
	...
	public static InitDriverPtr init_hardhedb = new InitDriverPtr() { public void handler()
	{
		/* patch ROM checksum (ROM1 fails self test) */
		memory_region( REGION_CPU1 ).write(0x1e5b, 0xAF);
	} };
	...
	***************************************************************************/
	...
	{
		UBytePtr	RAM	=	new UBytePtr(memory_region(REGION_CPU1));
		int	size	=	memory_region_length(REGION_CPU1)/2;
		int i;
	
		memory_set_opcode_base(0, new UBytePtr(RAM, size));
	...
	RAM.write(0x3337+size, 0xc9);	// RET Z -> RET (to avoid: jp $C800)
	//RAM[0x3338+size] = 0x00;	// jp $C800 -> NOP
	//RAM[0x3339+size] = 0x00;	// jp $C800 -> NOP
	//RAM[0x333a+size] = 0x00;	// jp $C800 -> NOP
	
	RAM.write(0x1406+size, 0x00);	// HALT -> NOP (NMI source??)
	RAM.write(0x2487+size, 0x00);	// HALT -> NOP
	RAM.write(0x256c+size, 0x00);	// HALT -> NOP
	} };
	...
	***************************************************************************/
	...
	public static int hardhea2_decrypt(int x, int encry, int mask)
	{
			switch( encry )
			{
			case 1:
	...
	// Address lines scrambling
			...
			RAM.write(i+size, hardhea2_decrypt(x,encry,mask));
		}
		...
		for (i = 0; i < 0x8000; i++)
		{
			...
			RAM.write(i, hardhea2_decrypt(x,encry,mask));
		}
	...
	*/
			...
			case 0x3d000:
				...
				{
					int j = (i & ~0xc0) | 0x80;
					x				=	RAM.read(j+0x10000);
					RAM.write(j+0x10000,	RAM.read(i+0x10000));
					RAM.write(i+0x10000,	x);
				}
			}
		}
	} };
	...
	***************************************************************************/
	...
	/* SAME AS HARDHEA2 */
	public static int starfigh_decrypt(int x, int encry, int mask)
	{
			switch( encry )
			{
			case 1:
	...
	// Address lines scrambling
			...
			RAM.write(i+size, starfigh_decrypt(x,encry,mask));
		}
		...
		for (i = 0; i < 0x8000; i++)
		{
			...
			RAM.write(i, starfigh_decrypt(x,encry,mask));
		}
	} };
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr hardhead_protection_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		if ((protection_val & 0x80)!=0)
			return	((~offset & 0x20)!=0			?	0x20 : 0) |
					((protection_val & 0x04)!=0	?	0x80 : 0) |
					((protection_val & 0x01)!=0	?	0x04 : 0);
		else
			return	((~offset & 0x20)!=0					?	0x20 : 0) |
					(((offset ^ protection_val) & 0x01)!=0	?	0x84 : 0);
	} };
	...
	*/
	public static WriteHandlerPtr hardhead_bankswitch_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ((data & ~0xef)!=0) 	logerror("CPU #0 - PC %04X: unknown bank bits: %02X\n",cpu_get_pc(),data);
	
		RAM = new UBytePtr(RAM, 0x4000 * bank + 0x10000);
		cpu_setbank(1, RAM);
	} };
	...
	*/
	public static WriteHandlerPtr rranger_bankswitch_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		UBytePtr RAM = new UBytePtr(memory_region(REGION_CPU1));
		int bank = data & 0x07;
		if ((~data & 0x10)!=0 && (bank >= 4))	bank += 4;
	
		if ((data & ~0xf7)!=0) 	logerror("CPU #0 - PC %04X: unknown bank bits: %02X\n",cpu_get_pc(),data);
	...
	*/
	public static WriteHandlerPtr brickzn_palettebank_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ((data & ~0x02)!=0) 	logerror("CPU #0 - PC %04X: unknown palettebank bits: %02X\n",cpu_get_pc(),data);
	...
	*/
	public static WriteHandlerPtr brickzn_spritebank_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ((data & ~0x03)!=0) 	logerror("CPU #0 - PC %04X: unknown spritebank bits: %02X\n",cpu_get_pc(),data);
	...
	*/
	public static WriteHandlerPtr brickzn_rombank_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ((data & ~0x0f)!=0) 	logerror("CPU #0 - PC %04X: unknown rom bank bits: %02X\n",cpu_get_pc(),data);
	...
	***************************************************************************/
	...
	public static WriteHandlerPtr hardhea2_nmi_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ((data & ~0x01)!=0) 	logerror("CPU #0 - PC %04X: unknown nmi bits: %02X\n",cpu_get_pc(),data);
	} };
	...
	*/
	...
	{
		flip_screen_set(data & 0x01);
		if ((data & ~0x01)!=0) 	logerror("CPU #0 - PC %04X: unknown flipscreen bits: %02X\n",cpu_get_pc(),data);
	} };
...
/*TODO*///		set_led_status(1, data & 0x02);
		coin_counter_w.handler(0, data & 0x04);
		if ((data & ~0x07)!=0)	logerror("CPU#0  - PC %06X: unknown leds bits: %02X\n",cpu_get_pc(),data);
	} };
	...
	*/
	...
	{
		suna8_spritebank = (data >> 1) & 1;
		if ((data & ~0x02)!=0) 	logerror("CPU #0 - PC %04X: unknown spritebank bits: %02X\n",cpu_get_pc(),data);
	} };
	...
	***************************************************************************/
	...
	public static WriteHandlerPtr starfigh_spritebank_latch_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if ((data & ~0x04)!=0) 	logerror("CPU #0 - PC %04X: unknown spritebank bits: %02X\n",cpu_get_pc(),data);
	} };
	...
	***************************************************************************/
	...
	static WriteYmHandlerPtr soundirq = new WriteYmHandlerPtr() {
	...
	***************************************************************************/
	...
	static YM3812interface hardhead_ym3812_interface = new YM3812interface
	(
		1,
		4000000,	/* ? */
		new int[]{ 50 },
		new WriteYmHandlerPtr[]{  null }		/* IRQ Line */
	);
	...
	***************************************************************************/
	...
	/* 2203 + 8910 */
	static YM2203interface rranger_ym2203_interface = new YM2203interface
	(
		2,
		4000000,	/* ? */
		new int[]{ YM2203_VOL(50,50), YM2203_VOL(50,50) },
		new ReadHandlerPtr[]{ null, null },	/* Port A Read  */
		new ReadHandlerPtr[]{ null, null },	/* Port B Read  */
		new WriteHandlerPtr[]{ null, null },	/* Port A Write */
		new WriteHandlerPtr[]{ null, null },	/* Port B Write */
		new WriteYmHandlerPtr[]{ null, null }		/* IRQ handler  */
	);
	...
	***************************************************************************/
	...
	static YM3812interface brickzn_ym3812_interface = new YM3812interface
	(
		1,
		4000000,	/* ? */
		new int[]{ 33 },
	...
	{
		if (cpu_getiloops() != 0)	return Z80_NMI_INT;
	...
	***************************************************************************/
	...
	{
		if (cpu_getiloops()!=0)
			if (suna8_nmi_enable!=0)	return Z80_NMI_INT;
	...
	***************************************************************************/
	...
	static YM3812interface starfigh_ym3812_interface = new YM3812interface
	(
		1,
		4000000,	/* ? */
		new int[]{ 50 },
