...
public class gb
{
	...
	//			spal = (oam[3] & 0x10) ? gb_spal1 : gb_spal0;
				...
				{
					data = (new UShortPtr(gb_ram, VRAM + (oam.read(2) & tilemask) * 16 + (height - 1 - line + oam.read(0)) * 2)).read(0);
				}
				else
				{
	...
	public static void gb_refresh_scanline ()
	{
		...
		while (l < 2)
		{
			...
			i = layer[l].xend;
                        ...
                        /*
                        System.out.println("xidx "+xidx);
                        System.out.println("map longo "+map.memory.length);
                        System.out.println("A "+map.memory[xidx]);
                        System.out.println("B "+(map.memory[xidx] ^ gb_tile_no_mod) * 8);
