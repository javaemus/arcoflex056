...
public class border
{
	...
	public static void draw_border(mame_bitmap bitmap,
	        ...
	        int EventID)                    /* Event ID of border messages */
	{
	        ...
	        else
	        {
	                ...
	                for (ScrY = 0; ScrY < TopBorderLines; ScrY++)
	                {
	                        ...
	                        else
	                        {
	                                ...
	                                ScrX = (int) ((int)(pItem[CurrItem].Event_Time - CyclesSoFar) * (float)TotalScreenWidth / (float)DisplayCyclesPerLine);
	                                r.max_x = ScrX-1;
	                                fillbitmap(bitmap, Machine.pens[CurrBorderColor], r);
	                                CurrBorderColor = pItem[CurrItem].Event_Data;
	                                do {
	                                ...
	                                {
	                                        NextScrX = (int) ((int)(pItem[CurrItem].Event_Time - CyclesSoFar) * (float)TotalScreenWidth / (float)DisplayCyclesPerLine);
	                                        r.min_x = ScrX;
	                                        r.max_x = NextScrX-1;
	                                        fillbitmap(bitmap, Machine.pens[CurrBorderColor], r);
	                                        ScrX = NextScrX;
	                ...
	                for (ScrY = TopBorderLines; ScrY < (TopBorderLines+ScreenLines); ScrY++)
	                {
	                        ...
	                        {
	                                /* Multiple colours */
	                                ScrX = (int) ((int)(pItem[CurrItem].Event_Time - CyclesSoFar) * (float)LeftBorderPixels / (float)LeftBorderCycles);
	                                r.max_x = ScrX-1;
	                                fillbitmap(bitmap, Machine.pens[CurrBorderColor], r);
	                                CurrBorderColor = pItem[CurrItem].Event_Data;
	                                do {
	                                ...
	                                {
	                                        NextScrX = (int) ((int)(pItem[CurrItem].Event_Time - CyclesSoFar) * (float)LeftBorderPixels / (float)LeftBorderCycles);
	                                        r.min_x = ScrX;
	                                        r.max_x = NextScrX-1;
	                                        fillbitmap(bitmap, Machine.pens[CurrBorderColor], r);
	                                        ScrX = NextScrX;
	                        ...
	                        {
	                                /* Multiple colours */
	                                ScrX = (int) (LeftBorderPixels + ScreenPixels + (int)(pItem[CurrItem].Event_Time - CyclesSoFar) * (float)RightBorderPixels / (float)RightBorderCycles);
	                                r.max_x = ScrX-1;
	                                fillbitmap(bitmap, Machine.pens[CurrBorderColor], r);
	                                CurrBorderColor = pItem[CurrItem].Event_Data;
	                                do {
	                                ...
	                                while ((CurrItem < NumItems) && (pItem[CurrItem].Event_Time < (CyclesSoFar+DisplayCyclesPerLine)))
	                                {
	                                        NextScrX = (int) (LeftBorderPixels + ScreenPixels + (int)(pItem[CurrItem].Event_Time - CyclesSoFar) * (float)RightBorderPixels / (float)RightBorderCycles);
	                                        r.min_x = ScrX;
	                                        r.max_x = NextScrX-1;
	                                        fillbitmap(bitmap, Machine.pens[CurrBorderColor], r);
	                                        ScrX = NextScrX;
	        ...
	        EventList_SetOffsetStartTime ( TIME_TO_CYCLES(0,cpu_getscanline()*cpu_getscanlineperiod()) );
                ...
                //System.out.println("MIN X: "+r.min_x);
                //System.out.println("MAX X: "+r.max_x);
	}
}
