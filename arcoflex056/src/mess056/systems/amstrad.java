...
public class amstrad
{
	...
	static long amstrad_cycles_at_frame_end = 0;
	/* cycle count of last write */
	static long amstrad_cycles_last_write = 0;
	static long time_delta_fraction = 0;
	
	static void amstrad_update_video()
	{
		int current_time;
		int time_delta;
	
		/* current cycles */
		current_time = (int) (TIME_TO_CYCLES(0,cpu_getscanline()*cpu_getscanlineperiod()) + amstrad_cycles_at_frame_end);
		/* time between last write and this write */
		time_delta = (int) (current_time - amstrad_cycles_last_write + time_delta_fraction);
	...
	*/
	...
	public static WriteHandlerPtr amstrad_ppi_portc_w = new WriteHandlerPtr() {
		...
		{
				device_output(IO_CASSETTE, 0, (data & (1<<5))!=0 ? -32768 : 32767);
		}
	...
	static ppi8255_interface amstrad_ppi8255_interface = new ppi8255_interface
	(
		1,
		new ReadHandlerPtr[]{amstrad_ppi_porta_r},
		new ReadHandlerPtr[]{amstrad_ppi_portb_r},
		new ReadHandlerPtr[]{null},
		new WriteHandlerPtr[]{amstrad_ppi_porta_w},
		new WriteHandlerPtr[]{null},
		new WriteHandlerPtr[]{amstrad_ppi_portc_w}
	);
	...
	static UBytePtr[] AmstradCPC_RamBanks = new UBytePtr[4];
        static {
            for (int i = 0; i<4 ; i++)
                AmstradCPC_RamBanks[i] = new UBytePtr();
        }
	
	/* base of all ram allocated - 128k */
	public static UBytePtr Amstrad_Memory = new UBytePtr();
	
	/* current selected upper rom */
	public static UBytePtr Amstrad_UpperRom = new UBytePtr();
	
	/* bit 0,1 = mode, 2 = if zero, os rom is enabled, otherwise
	disabled, 3 = if zero, upper rom is enabled, otherwise disabled */
	public static int AmstradCPC_GA_RomConfiguration;
	
	public static int[] AmstradCPC_PenColours = new int[18];
	...
	/* ram configuration */
	public static int AmstradCPC_GA_RamConfiguration;
	
	/* selected pen */
	public static int AmstradCPC_GA_PenSelected;
	
	
	
	public static int AmstradCPC_ReadKeyboard()
	{
	...
	public static void Amstrad_RethinkMemory()
	{
		...
		{
			UBytePtr BankBase = new UBytePtr();
	...
	{
		Amstrad_UpperRom = new UBytePtr(Amstrad_ROM_Table[Data & 0x0ff]);
	
		logerror("upper rom %02xn",Data);
	...
	static int previous_printer_data_byte;
	...
	public static WriteHandlerPtr AmstradCPC_WritePortHandler = new WriteHandlerPtr() {
		...
		if ((offset & 0x04000) == 0)
		{
			...
			case 1:
				{
	/*TODO*///#if 0
	/*TODO*///				int current_time;
	/*TODO*///								int time_delta;
	/*TODO*///								int cur_time;
	/*TODO*///
	/*TODO*///								/* current time */
	/*TODO*///								current_time = TIME_TO_CYCLES(0,cpu_getscanline()*cpu_getscanlineperiod());
	/*TODO*///								cur_time = current_time;
	/*TODO*///
	/*TODO*///								if (previous_crtc_write_time>current_time)
	/*TODO*///								{
	/*TODO*///									cur_time += cpu_getfperiod();
	/*TODO*///								}
	/*TODO*///
	/*TODO*///								/* time between last write and this write */
	/*TODO*///								time_delta = (cur_time - previous_crtc_write_time)>>2;
	/*TODO*///
	/*TODO*///								/* set new previous write */
	/*TODO*///								previous_crtc_write_time = current_time;
	/*TODO*///
	/*TODO*///#ifdef AMSTRAD_VIDEO_EVENT_LIST
	/*TODO*///				/* crtc register write */
	/*TODO*///				{
	/*TODO*///
	/*TODO*///					EventList_AddItemOffset((EVENT_LIST_CODE_CRTC_WRITE<<6), data, TIME_TO_CYCLES(0,cpu_getscanline()*cpu_getscanlineperiod()));
	/*TODO*///				}
	/*TODO*///#endif
	/*TODO*///								/* recalc time */
	/*TODO*///								crtc6845_recalc(0, time_delta);
	/*TODO*///
	/*TODO*///#else
									  amstrad_update_video();
	/*TODO*///#endif
	...
	static UBytePtr multiface_ram = new UBytePtr();
	static long multiface_flags;
	
	/* stop button has been pressed */
	public static int MULTIFACE_STOP_BUTTON_PRESSED	= 0x0001;
	/* ram/rom is paged into memory space */
	public static int MULTIFACE_RAM_ROM_ENABLED     = 0x0002;
	/* when visible OUT commands are performed! */
	public static int MULTIFACE_VISIBLE             = 0x0004;
	...
	public static void multiface_init()
	{
	...
	public static void multiface_exit()
	{
		/* free ram */
	...
	public static void multiface_reset()
	{
	...
	{
			UBytePtr multiface_rom = new UBytePtr();
			...
			)
		{
			...
			/* set bank addressess */
			cpu_setbank(1, new UBytePtr(multiface_rom));
			cpu_setbank(2, new UBytePtr(multiface_ram));
			cpu_setbank(9, new UBytePtr(multiface_rom));
			cpu_setbank(10, new UBytePtr(multiface_ram));
		}
	}
	...
	*/
	...
	public static void amstrad_interrupt_timer_trigger_reset_by_vsync()
	{
	...
	public static int US_TO_CPU_CYCLES(int x){
            return ((x)<<2);
        }
	...
	public static void amstrad_common_init()
	{
		...
		/*TODO*///previous_op_table = cpunum_get_cycle_table(0,Z80_TABLE_op);
		/*TODO*///previous_cb_table = cpunum_get_cycle_table(0,Z80_TABLE_cb);
		/*TODO*///previous_ed_table = cpunum_get_cycle_table(0,Z80_TABLE_ed);
		/*TODO*///previous_xy_table = cpunum_get_cycle_table(0,Z80_TABLE_xy);
		/*TODO*///previous_xycb_table = cpunum_get_cycle_table(0,Z80_TABLE_xycb);
		/*TODO*///previous_ex_table = cpunum_get_cycle_table(0,Z80_TABLE_ex);
	
		/* Using the cool code Juergen has provided, I will override
		the timing tables with the values for the amstrad */
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_op, amstrad_cycle_table_op);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_cb, amstrad_cycle_table_cb);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_ed, amstrad_cycle_table_ed);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_xy, amstrad_cycle_table_xy);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_xycb, amstrad_cycle_table_xycb);
	...
	public static void amstrad_shutdown_machine()
	{
		...
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_op, previous_op_table);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_cb, previous_cb_table);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_ed, previous_ed_table);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_xy, previous_xy_table);
		/*TODO*///cpunum_set_cycle_tbl(0,Z80_TABLE_xycb, previous_xycb_table);
	...
	static Memory_ReadAddress readmem_amstrad[]
            = {
                new Memory_ReadAddress(MEMPORT_MARKER, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_8),
                new Memory_ReadAddress(0x00000, 0x01fff, MRA_BANK1),
		new Memory_ReadAddress(0x02000, 0x03fff, MRA_BANK2),
		new Memory_ReadAddress(0x04000, 0x05fff, MRA_BANK3),
		new Memory_ReadAddress(0x06000, 0x07fff, MRA_BANK4),
		new Memory_ReadAddress(0x08000, 0x09fff, MRA_BANK5),
		new Memory_ReadAddress(0x0a000, 0x0bfff, MRA_BANK6),
		new Memory_ReadAddress(0x0c000, 0x0dfff, MRA_BANK7),
		new Memory_ReadAddress(0x0e000, 0x0ffff, MRA_BANK8),
                new Memory_ReadAddress(MEMPORT_MARKER, 0) /* end of table */};
	
	static Memory_WriteAddress writemem_amstrad[]
            = {
                new Memory_WriteAddress(MEMPORT_MARKER, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_8),
                new Memory_WriteAddress(0x00000, 0x01fff, MWA_BANK9),
		new Memory_WriteAddress(0x02000, 0x03fff, MWA_BANK10),
		new Memory_WriteAddress(0x04000, 0x05fff, MWA_BANK11),
		new Memory_WriteAddress(0x06000, 0x07fff, MWA_BANK12),
		new Memory_WriteAddress(0x08000, 0x09fff, MWA_BANK13),
		new Memory_WriteAddress(0x0a000, 0x0bfff, MWA_BANK14),
		new Memory_WriteAddress(0x0c000, 0x0dfff, MWA_BANK15),
		new Memory_WriteAddress(0x0e000, 0x0ffff, MWA_BANK16),
	...
	static IO_ReadPort readport_amstrad[]
            = {
                new IO_ReadPort(MEMPORT_MARKER, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_IO | MEMPORT_WIDTH_8),
                new IO_ReadPort(0x0000, 0x0ffff, AmstradCPC_ReadPortHandler),
                new IO_ReadPort(MEMPORT_MARKER, 0) /* end of table */};
	
	static IO_WritePort writeport_amstrad[]
            = {
                new IO_WritePort(MEMPORT_MARKER, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_IO | MEMPORT_WIDTH_8),
                new IO_WritePort(0x0000, 0x0ffff, AmstradCPC_WritePortHandler),
	...
	public static void KEYBOARD_PORTS(){
		/* keyboard row 0 */ 
		PORT_START();
	...
	public static void MULTIFACE_PORTS(){
		PORT_START();
	...
	static MachineDriver machine_driver_cpcplus = new MachineDriver(
            /* basic machine hardware */
            new MachineCPU[]{
                new MachineCPU(
				CPU_Z80_MSX | CPU_16BIT_PORT,  /* type */
	...
	***************************************************************************/
	...
	static IODevice AMSTRAD_IO_SNAPSHOT = new IODevice
		( 
			IO_SNAPSHOT,				/* type */ 
			1,							/* count */ 
...
/*TODO*///		AMSTRAD_IO_PRINTER,
	...
	/*TODO*///#define io_kccomp io_cpc6128
	/*TODO*///#define io_cpc464 io_cpc6128
	/*TODO*///#define io_cpc664 io_cpc6128
...
/*TODO*///		AMSTRAD_IO_PRINTER,
	...
	/*TODO*///#define io_cpc6128p io_cpcplus
	/*TODO*///#define io_cpc464p io_cpcplus
	...
	public static GameDriver driver_cpc464p = new GameDriver("1990", "cpc464p", "amstrad.java", rom_cpc464p, null, machine_driver_cpcplus, input_ports_amstrad, null, io_cpcplus, "Amstrad plc", "Amstrad 464plus");
        //COMP( 1990, cpc6128p,  0,		cpcplus,  amstrad,	0,	 "Amstrad plc", "Amstrad 6128plus")
	public static GameDriver driver_cpc6128p = new GameDriver("1990", "cpc6128p", "amstrad.java", rom_cpc6128p, null, machine_driver_cpcplus, input_ports_amstrad, null, io_cpcplus, "Amstrad plc", "Amstrad 6128plus");
